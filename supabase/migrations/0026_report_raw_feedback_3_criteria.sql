-- 0026: v_report_raw — thay cột "CSAT Rating" gộp bằng 3 cột riêng theo
-- đúng 3 tiêu chí đánh giá mới (xem 0025):
--   "Thời gian hỗ trợ"
--   "Thái độ nhân viên hỗ trợ"
--   "Vấn đề Đối tác đã được ghi nhận hỗ trợ đầy đủ"
--
-- Trang Admin → Export dùng select("*") động trên view này nên không cần
-- sửa code, chỉ cần đổi cấu trúc view. Đổi tên cột giữa các cột hiện có
-- không được phép qua CREATE OR REPLACE VIEW nên phải DROP + CREATE lại
-- (Postgres tự khôi phục grant mặc định cho anon/authenticated/service_role).
